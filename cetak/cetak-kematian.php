...
// batas
if (isset($_POST['tgl_mati'])) {
	$tgl_mati = $_POST['tgl_mati'];
}
else{
	echo "kesalahan dari tgl_mati";
}
// batas
if (isset($_POST['nama_lapor'])) {
	$nama_lapor = $_POST['nama_lapor'];
}
else{
	echo "kesalahan dari nama_lapor";
}
// batas
if (isset($_POST['nik_lapor'])) {
	$nik_lapor = $_POST['nik_lapor'];
}
else{
	echo "kesalahan dari nik_lapor";
}
// batas
...
$nama_up_lapor = strtoupper($nama_lapor);
$ttl_new = "$hari_lahir $bulan_lahir $tahun_lahir";
$tgl_meninggal = "$hari_mati $bulan_mati $tahun_mati";
...
$pdf->SetFont('Times','B',12);
$pdf->Ln(5);
$pdf->Cell(0,0,"Nomor: 471.5/$no_surat/431.513.9.2/$tahun",0,1,'C');
// isi
$pdf->Ln(10);
$pdf->Cell(5);
...
$pdf->Cell(5,7,':',0,0,'L');
$pdf->SetFont('Times','B',12);
$pdf->MultiCell(0,7,"Tersebut diatas benar-benar telah meninggal dunia pada hari ".sebutHari($tgl_mati).", $tgl_meninggal di Desa Bantal Kecamatan Asembagus.",0,'J');

$pdf->Cell(8);
$pdf->SetFont('Times','',12);
$pdf->Cell(40,7,'Penyebab Kematian',0,0,'L');
$pdf->Cell(5,7,':',0,0,'L');
$pdf->Cell(0,7,ucwords($sebab),0,1,'L');

$pdf->Cell(8);
$pdf->Cell(40,7,'Pelapor',0,0,'L');
$pdf->Cell(5,7,':',0,0,'L');
$pdf->SetFont('Times','B',12);
$pdf->Cell(0,7,$nama_up_lapor,0,1,'L');

$pdf->Cell(8);
$pdf->SetFont('Times','',12);
$pdf->Cell(40,7,'NIK',0,0,'L');
$pdf->Cell(5,7,':',0,0,'L');
$pdf->Cell(0,7,$nik_lapor,0,1,'L');

$pdf->Ln(10);
$pdf->Cell(15);
$pdf->Cell(0,7,"Demikian Surat Keterangan  ini dibuat untuk digunakan sebagaimana mestinya.",0,1,'J');
...
$pdf->Cell(65,6,$nama_up_lapor,0,0,'C');
$pdf->Cell(38,6,'',0,0,'L');
